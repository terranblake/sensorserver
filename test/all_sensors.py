...
# Configure the state_data_logger from sensor_logic
state_data_logger = logging.getLogger("state_data")
state_data_file_handler = logging.FileHandler("state_data.log", mode='a')
state_data_file_handler.setFormatter(logging.Formatter('%(message)s')) # Only message (JSON Lines)
state_data_logger.addHandler(state_data_file_handler)
...
async def display_simple_status():
    ...
    while True:
        ...
        print(f"(Updating every {TERMINAL_REFRESH_RATE}s, script log: {LOG_FILE}, raw: raw_data.log, state: state_data.log)", flush=True)
